...
ensure_ready() {
  [ -d "$WEB_DIR/.venv" ] || { echo "[run-all] web/.venv missing. Run bash scripts/setup/reset_ws.sh first." >&2; exit 1; }
  source_if_exists "/opt/ros/${ROS_DISTRO_REQUIRED}/setup.bash" || { echo "[run-all] ROS Jazzy setup missing." >&2; exit 1; }
  source_if_exists "$ROOT_DIR/install/setup.bash" || { echo "[run-all] workspace setup missing. Run bash scripts/setup/reset_ws.sh first." >&2; exit 1; }
  pg_isready -d "$DATABASE_URL" >/dev/null 2>&1 || { echo "[run-all] PostgreSQL/DB not ready. Run bash scripts/setup/reset_ws.sh first." >&2; exit 1; }
}
