...
1. **DOCK_IN 실패 시 도크 점유 해제 정책 (의도된 한계)**
   ...
   - dock 해제 이벤트도 외부에 통지하지 않는다 — Fleet API 는 state_manager 가
   ...
   - 운영상 leak 이 관측되면 안전망의 트리거 조건(`prev == 'CHARGING'`) 을
     확장하거나 Fleet API / TaskManager 가 상위 흐름에서 정리한다.
...
- 2026-05-24: dock 점유 정보(`_robot_dock`)는 TrafficManager 단독 보유로 확정.
  경로 탐색·dock 선정 모두 TrafficManager 책임이므로 Fleet API / DB /
