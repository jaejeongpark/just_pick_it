...
이 문서는 PICKY/COBOT State Manager 담당자가 Fleet Manager와 맞춰야 하는 실행 계약을 정리한다.
코드 구조와 리뷰 기준은 `FLEET_MANAGER_CODE_REVIEW.md`를 보고, 이 문서는 실제 로봇 쪽에서
언제 어떤 상태, feedback, result를 보내야 하는지 확인할 때 사용한다.
...
FleetRepository는 DB 조회/쓰기를 담당한다.
Fleet API는 Web Gateway와 외부 확인용 HTTP/WebSocket 통신을 담당한다.
```

## 기본 원칙

- PICKY/COBOT은 Web Gateway나 Fleet API를 직접 호출하지 않는다.
...
Fleet Manager는 Fleet API의 emergency/resume 요청을 받으면 각 State Manager에 ROS2 service를 호출한다.
...
3. emergency 중인 action을 성공으로 반환하지 않는다.
...
- emergency 상태에서 기존 action을 pause할지 cancel/fail할지는 State Manager 구현 정책으로 정한다.
- 성공하지 않은 물리 동작을 SUCCESS로 보고하면 안 된다.

## Fleet Manager 내부 호출 경로

State Manager가 직접 Fleet Manager의 Python 함수를 호출하는 것은 아니다.
실제 연결은 `RobotCommandGateway`와 `RobotStateMonitor`가 담당한다.
...
PICKY Action result
  ...
  -> FleetRepository.update_task_status(...)
  -> Fleet API WebSocket snapshot push
...
- `CHARGING_DOCK_*`는 DB zone pose가 아니라 TrafficManager/State Manager가 공유하는 논리 도크 이름이다.
...
MOVE_TO_PRODUCT result SUCCESS
  -> PICKY는 물건 앞에 도착한 상태
  -> 다음 COBOT task가 있으면 FleetRepository가 PICKY를 WAITING_FOR_COBOT으로 표시
  -> Web UI는 Fleet API snapshot/WebSocket을 통해 이 상태를 본다
...
현재 저장소에는 COBOT용 `ExecuteTask.action`이 아직 정의되어 있지 않다.
COBOT 담당자가 action을 정의하면 Fleet Manager의 `RobotCommandGateway.send_cobot_task()`에 연결한다.
...
```text
1. Fleet Manager ROS2 node
2. Fleet API (:8100)
3. Web Gateway (:8000)
```

Web만 단독으로 띄울 때는 `web/scripts/run.sh`를 사용한다.
단, Web Gateway는 Fleet API를 프록시하므로 실제 화면 데이터는 Fleet Manager가 떠 있어야 정상 조회된다.

```bash
web/scripts/run.sh
```

현재 저장소에서 로봇 실행까지 성공으로 확인하려면 다음 action/service server가 필요하다.
...
이 서버들이 없으면 주문 생성과 task 생성은 확인할 수 있지만, dispatch 단계에서 action server 없음으로 task가 실패한다.
이 실패는 Fleet Manager 자체 오류가 아니라 로봇 쪽 실행 서버가 아직 없다는 의미다.
...
2. just_pick_it_interfaces/CMakeLists.txt에 ExecuteTask.action 등록
3. RobotCommandGateway.send_cobot_task() 구현
4. COBOT feedback에서 STOWING_ARM 감지
5. STOWING_ARM 감지 시 TaskManager.preplan_after_cobot_stowing(task_id) 호출
6. COBOT result SUCCESS는 STOWING_ARM 완료 후에만 반환
7. RobotStateMonitor 또는 별도 monitor의 cobot_state / battery / pose / task event 수신 확장
8. 전체 연동 테스트용 fake 또는 실제 action/service server 준비
```
