...
| 2 | 관제 PC | `./reset_demo_data.sh && ./run_all.sh` — DB 리셋 후 Fleet(:8100)+Web(:8000) 통합 실행 |
| 3 | 브라우저 | 주문은 **http://localhost:8000/customer** , 상태 확인은 **http://localhost:8000/admin** |
...
cd ~/just_pick_it
git pull origin dev
./build_picky1.sh              # PICKY1 실로봇 bringup/SLAM/Nav2/State Manager 관련 패키지만 빌드
...
cd ~/just_pick_it
git pull origin dev
./build_picky2.sh              # PICKY2 실로봇 bringup/SLAM/Nav2/State Machine 관련 패키지만 빌드
source install/setup.bash
```
...
```bash
./build_picky1.sh --clean       # PICKY1 보드
./build_picky2.sh --clean       # PICKY2 보드
```

`build_amr.sh` 는 AMR 계열 패키지를 넓게 빌드하는 공통 스크립트다. 실로봇 한 대만 빠르게 확인할 때는
위의 `build_picky1.sh` / `build_picky2.sh` 를 우선 사용한다.
...
| 증상 | 원인 | 해결 |
|---|---|---|
| `urdf_tutorial`/`moveit_msgs` 못 찾아 빌드 실패 | AMR 보드가 로봇팔 패키지까지 빌드 | PICKY1은 `./build_picky1.sh`, PICKY2는 `./build_picky2.sh` 사용 |
| `existing path cannot be removed: Is a directory` | 예전 `--symlink-install` 산출물 충돌 | `./build_picky1.sh --clean` 또는 `./build_picky2.sh --clean` 후 재빌드 |
