...
- **DB는 Control Server가 단독 소유**했다. SQLAlchemy `engine` / `SessionLocal` (`web/app/database.py`),
  ORM 모델(`web/app/models.py`), 비즈니스 로직(`web/app/services/*`)이 모두 웹 프로세스 안에 있었다.
- **Fleet Manager(ROS2)는 DB에 직접 닿지 못했다.** 모든 읽기/쓰기를 Control Server의 HTTP API로 처리했다.
  접점은 `src/.../fleet_manager/control_server_client.py`의 `ControlServerClient` 하나로 모여 있었다.
- Control Server는 `/api/fleet/*` 네임스페이스에 Fleet Manager 전용 브리지 엔드포인트를 두고 있었다
...
즉 Fleet Manager가 DB 한 줄을 읽거나 쓸 때마다 **네트워크 왕복 + JSON 직렬화/역직렬화 + FastAPI 라우팅**을 거쳤다.
...
`- Fleet API (HTTP/REST + WebSocket)
       ...
       | HTTP/REST + WebSocket
       v
Web Gateway Process (프레젠테이션 + proxy 전용)
...
`- /api/* WebSocket proxy
       ...
       | PostgreSQL
       v
DB  <--- Fleet Manager만 직접 접속
```

- 웹은 별도 프로세스로 유지하되 **DB 연결 문자열·접속 권한을 갖지 않는다.**
- 쓰기(주문 생성, emergency-stop 등)는 웹이 Fleet API에 명령을 전달하고 Fleet Manager가 DB에 쓴다.
- 읽기(화면 표시)도 웹이 Fleet API에 조회를 요청한다. 즉 **읽기·쓰기 모두 Fleet Manager 경유**다.

### 2.2 이 구조의 장점

- **DB 동시 접근 정합성 문제가 원천적으로 줄어든다.** 커넥션이 Fleet Manager 계층으로 모이므로 race/덮어쓰기 위험이 작아진다.
- 비즈니스 로직(상태 전이 규칙 등)이 Fleet Manager 한 곳에 모인다.
...
```text
브라우저 -> Web Gateway(페이지 + proxy) -> Fleet Manager API -> DB
```
...
| 실시간 푸시 (스트림) | WebSocket | 로봇 위치·상태·주문 상태 변화 |
...
> 로봇을 실제로 움직이는 동작은 HTTP 스레드에서 rclpy를 직접 호출하지 말고 Fleet Manager node 메서드로 위임한다.
...
`web/app/models.py`(테이블 정의)와 `web/app/services/*`(상태 전이·스냅샷 로직)은 이미 동작하며 검증된 자산이었다.
...
- 웹이 더 이상 DB를 쓰지 않으므로, 원래 고려했던 "웹·Fleet 공용 패키지로 공유" 대신
  **DB 코드를 Fleet Manager 안으로 이사**시키면 된다. 웹에는 DB 코드가 남지 않는다.
...
| 기존 HTTP 엔드포인트 | 기존 ControlServerClient 메서드 | 흡수 후 |
|---|---|---|
...
- `workflow_service.py` - task <-> order <-> robot 상태 전이의 단일 진실 원천.
- `status_service.py` - admin/customer 스냅샷 빌더.
- `stocking_service.py`, `inventory_status.py`, `robot_runtime_policy.py`.

### 4.2 Fleet Manager로 위임할 것 (기존 웹이 직접 DB에 쓰던 동작)

웹은 DB에 못 쓰므로, 아래 동작은 **웹이 Fleet API에 명령을 보내고 Fleet Manager가 DB에 쓴다** (Web Gateway proxy 경유).

- **고객 주문 생성** (`POST /api/orders`).
- **관리자 동작** - emergency-stop / resume / 상품 등록·재고 수정 / pickup-slot 생성 / 입고 요청.

### 4.3 웹에 남는 것 (UI / 외부 연동) - 단, DB 접근 없음

- Customer/Admin HTML 페이지 (`page_router.py`, `templates/`).
- `/api/*` HTTP/WebSocket proxy (`fleet_api_proxy_router.py`).
...
- [x] FastAPI 결합부(`get_db` 제너레이터)는 공용 패키지에 두지 않음. 세션 팩토리/`scoped_session`/`session_scope`만 노출(`just_pick_it_db/session.py`).
- [x] **스레드별 세션 관리** 적용: `get_scoped_session()` + `session_scope()` 컨텍스트 매니저, 풀 크기 env(`DB_POOL_SIZE`) 조정.
- [x] ROS2 빌드 의존성 해결: `package.xml`에 `python3-sqlalchemy`/`python3-psycopg2`, `fleet_manager`가 `just_pick_it_db` 의존.
- [x] 전환 중에는 기존 `app.models`/`app.services.*` 경로를 re-export shim으로 유지했고, Phase 5에서 Web Gateway가 Fleet API proxy 전용으로 바뀌면서 제거 완료.
...
- [x] not-found/검증 실패는 `RepoError` 또는 `None`/빈 list 반환으로 흡수해 기존 HTTP 4xx -> None 계약과 유사하게 유지.
- [x] 실 DB 대상 읽기·쓰기 검증 완료(로봇 상태 변경, task 생성/전이, 이벤트, 예외 생성, 실패 케이스).

> 메모: 데이터 read/write의 HTTP 의존은 제거됐다. emergency/resume 명령 경로는 Phase 4 Fleet API route로 전환 완료.
...
- [x] `get_order_work`/`get_stocking_work`의 다회 조회(주문+상품맵+zone맵)를 **단일 `session_scope` 트랜잭션**으로 합침. 일관된 스냅샷 읽기 + 세션/커넥션 1회.
- [ ] PostgreSQL `LISTEN/NOTIFY` 기반 이벤트화는 **Phase 4 이후로 의도적 보류**. 이유: Phase 4 이후 주문/입고 생성이 Fleet API로 위임되면 Fleet이 생성 사실을 이미 알고 있으므로, 외부 INSERT 감지를 위한 NOTIFY 자체가 불필요해질 가능성이 크다(현재 5초 polling으로 충분).

### Phase 4 - Fleet Manager API 서버 구축 (완료)

- [x] **HTTP API 서버 골격**(`fleet_api_server.py`): ROS2 노드 프로세스 안에서 uvicorn을 데몬 스레드로 기동(rclpy executor + asyncio 공존). 데몬 스레드 signal 핸들러 비활성화로 공존 문제 해결. 노드 `api_enabled/api_host/api_port` 파라미터, start/stop 수명주기 연결.
...
- [x] 입고 요청 생성 API: `POST /api/admin/stocking-items`가 `stocking_item`을 `REQUESTED` 상태로 생성한다.
- [x] ROS 핸드오프 명령(emergency-stop/resume): Fleet API -> `FleetManagerNode.trigger_emergency_stop()` -> DB 전이(`FleetRepository.apply_emergency_stop/apply_resume`) + `RobotCommandGateway` 서비스 전파 + `TaskManager` emergency gate 처리.
...
> 검증: 각 증분을 TestClient + 실기동(uvicorn 스레드 바인드/응답/정지, 실 WebSocket 클라이언트) + 실 DB로 확인.

### Phase 5 - 웹을 Fleet API 클라이언트로 전환 (완료, LLM 실제 parser 제외)

목표: 웹이 DB에 직접 접근하지 않고, 읽기·쓰기 모두 Fleet API(Phase 4)를 경유하게 한다. 끝나면 웹에 DB 코드가 0줄 남는다.

**5.0 전달 경로 결정 (완료)**

- [x] 브라우저 <-> Fleet 연결 방식은 **웹 프록시(A안)** 로 확정. 브라우저는 기존처럼 Web Gateway의 `/api/*`를 호출하고, Web Gateway가 Fleet API로 forward 한다.
- [x] Fleet API base URL을 웹 설정값으로 추가: `FLEET_API_BASE_URL`, `FLEET_API_WS_BASE_URL`.
...
- [x] Fleet API 연결 실패 시 Web Gateway는 503으로 응답한다.

**5.2 읽기 라우터 전환 (완료)**

- [x] 기존 product/order/customer/admin/status 조회 라우터를 DB 직접 조회 대신 `/api/{path:path}` proxy로 통합.
- [x] `GET /api/products`, `/api/orders`, `/api/customer/status`, `/api/admin/status` 등은 Fleet API로 전달된다.

**5.3 쓰기 라우터 전환 (완료)**

- [x] 주문 생성/완료, 관리자 상품/슬롯/예외/emergency/resume 요청도 Web Gateway에서 Fleet API로 전달된다.
- [x] Web Gateway는 요청 body/header를 넘기고, Fleet API 응답 status/body를 그대로 브라우저에 반환한다.

**5.4 WebSocket 전환 (완료)**

- [x] `customer`/`admin` 상태 WebSocket을 Fleet API WebSocket으로 프록시한다.
...
- [x] `web/requirements.txt`에서 `sqlalchemy`, `psycopg2` 제거. Web Gateway는 DB driver를 설치하지 않는다.
- [x] `web/scripts/setup.sh`는 Web Gateway venv/requirements/.env만 담당한다. DB/rosdep/colcon은 root script로 이동.
...
- [x] 웹 `fleet_router`(`/api/fleet/*`) 삭제: Fleet이 더 이상 웹을 호출하지 않으므로 제거 완료.
- [x] 노드의 Fleet event WebSocket listener 제거: `server_base_url`, `_start_fleet_event_listener`, `/api/fleet/ws/events` 수신 루프 제거. emergency/resume은 Fleet API -> node trigger -> RobotCommandGateway service 전파 경로로 통일.
- [ ] 아키텍처 문서(`docs/3_System_Architecture.pdf`) ver_3.0 갱신: Control Server를 DB 비접근 Web Gateway로 표기, DB 화살표는 Fleet Manager에만 연결.
- [x] 실행/운영 문서 갱신: `README.md`, `인수인계서.md`, `db/README.md`, `web/README.md`.
- [x] 전체 흐름 문서 추가: `docs/전체_워크플로.md`.
- [x] 코드리뷰/연동가이드 갱신: `src/just_pick_it/fleet_manager/FLEET_MANAGER_CODE_REVIEW.md`, `src/just_pick_it/fleet_manager/PICKY_COBOT_상태관리_연동가이드.md`, `reviews/fleet_manager/*`.
- [x] 역할 경계 문서 추가: `src/just_pick_it/fleet_manager/OWNERSHIP.md`.
- [ ] `TASK_PLAN.md` 최종 상태 정리.
...
- [x] root `reset_demo_data.sh` 추가: schema 전체가 아니라 데모 테이블/seed만 빠르게 되돌리는 용도.
- [x] `web/scripts/run.sh`는 Web Gateway only, `web/scripts/setup.sh`는 Web Gateway venv setup only로 책임 축소.
...
| API 서버·rclpy 공존 | HTTP/REST 방식(확정) 때문에 한 프로세스에서 uvicorn(asyncio)과 rclpy executor가 공존. uvicorn은 별도 스레드, 로봇 제어 동작은 node 메서드로 위임 (2.3). |
...
| 마이그레이션 후 잔여 확인 | 주요 read/write는 `FleetRepository`/Fleet API로 정리됐지만, 운영 DB 계정 권한 분리와 실로봇 E2E는 남은 확인 항목이다. |
...
1. **토폴로지**: Fleet Manager가 DB 단독 게이트웨이. 웹은 DB 비접근 Web Gateway로 분리, **읽기·쓰기 모두 Fleet Manager 경유**.
2. **전달 방식**: **HTTP/REST API + WebSocket** 확정. Fleet Manager가 API 서버를 열고 웹은 HTTP/WS proxy 역할을 한다.
3. **DB 접근 코드**: 기존 SQLAlchemy 자산을 `just_pick_it_db`로 이사해 Fleet Manager에서 재사용. 재작성하지 않는다.
4. **기술**: ORM(SQLAlchemy) 유지, `FleetRepository` 계층으로 감싼다.
5. **필수 안전장치**: 스레드별 세션 관리 + DB 계정 권한으로 웹 DB 접근 차단.
6. **이행 순서**: DB 코드 이사 -> 쓰기 DB 직접화 -> 읽기 DB 직접화 -> Fleet API 서버 구축 -> Web Gateway proxy 전환 -> 실행 스크립트/문서 정리 -> 남은 E2E/아키텍처 문서 갱신.
