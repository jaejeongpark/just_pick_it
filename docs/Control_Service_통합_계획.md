...
- [ ] 주문/입고 감지를 HTTP polling → DB polling으로 전환은 완료(이미 DB 조회). 추가로 PostgreSQL `LISTEN/NOTIFY` 기반 이벤트화는 후속 최적화로 남김.
- [ ] `get_order_work`의 다회 조회(주문+상품맵+zone맵)를 단일 JOIN으로 합치는 것은 후속 최적화로 남김(현재는 HTTP 제거된 다회 DB 조회).
