...
- [ ] ROS 핸드오프 명령(emergency-stop/resume): DB 변경 + RobotCommandGateway 전파 + task_manager.handle_*. uvicorn 스레드에서 executor 로 위임하는 구조 필요(다음 증분).
- [ ] LLM 명령(`POST /api/admin/llm/messages`): 현재 `llm_client` stub 이 web 잔류. LLM 담당 연결과 함께 처리(보류).
- [ ] 실시간 WebSocket push 추가: 내부 DB 갱신 지점에서 스냅샷 push(`realtime.py` 역할 이전).
