...
- [ ] Fleet Manager API 스펙 초안 작성 (조회 GET, 명령 POST, 실시간 WebSocket 엔드포인트 목록).
...
### Phase 1 — DB 코드 자산을 Fleet Manager로 이사

- [ ] `web/app/models.py` + `services/*`를 Fleet Manager가 쓰는 위치(공용 모듈 또는 ROS2 패키지 내부)로 이동.
- [ ] FastAPI 결합부(`get_db` 제너레이터) 제거, 세션 팩토리(`SessionLocal`)만 노출.
- [ ] **스레드별 세션 관리**(`scoped_session` 또는 콜백당 세션) 적용. 커넥션 풀 크기를 executor 스레드 수에 맞춤.
- [ ] ROS2 빌드에서 SQLAlchemy/psycopg 의존성 해결(rosdep/venv/`package.xml` 또는 `requirements`).

### Phase 2 — Fleet Manager 쓰기 경로 DB 직접화

- [ ] DAO/Repository 계층 도입. `ControlServerClient` 인터페이스는 유지한 채 내부 구현만 HTTP → DB로 교체.
- [ ] 4.1 표의 쓰기 메서드를 DB 직접 호출로 대체. 상태 전이는 반드시 공용 `workflow_service` 로직을 거치게 한다.
- [ ] 다중 테이블 동시 갱신(`stocking/complete` 등)은 단일 트랜잭션으로 처리.

### Phase 3 — Fleet Manager 읽기 경로 DB 직접화

- [ ] `get_snapshot/list_orders/list_zones/list_products/list_pickup_slots` 등을 DB SELECT로 대체.
- [ ] 주문/입고 감지를 HTTP polling → DB polling (또는 PostgreSQL `LISTEN/NOTIFY`)으로 전환.
- [ ] `get_order_work`의 다회 조회를 단일 트랜잭션 내 JOIN 조회로 통합.

### Phase 4 — 웹 DB 분리 (읽기·쓰기 모두 Fleet 경유로 전환)

- [ ] 웹에서 DB 연결 코드 제거 (`database.py`, 모델 import 등). 웹에 DB 코드 0줄.
- [ ] 웹의 쓰기 동작(주문 생성·관리자 명령)을 Fleet Manager 위임 호출로 교체 (4.2, 전달 방식 적용).
- [ ] 웹의 화면 데이터 조회도 Fleet Manager 경유 요청으로 교체 (직접 DB 조회 제거).
