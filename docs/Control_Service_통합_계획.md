...
### Phase 4 — 웹 DB 분리 (읽기·쓰기 모두 Fleet 경유로 전환)

- [x] **Fleet Manager HTTP API 서버 골격**(`fleet_api_server.py`): ROS2 노드 프로세스 안에서 uvicorn 을 데몬 스레드로 기동(rclpy executor + asyncio 공존). 데몬 스레드 signal 핸들러 비활성화로 공존 문제 해결. 노드 `api_enabled/api_host/api_port` 파라미터, start/stop 수명주기 연결.
  - 엔드포인트(읽기): `GET /api/health/db`, `/api/admin/status`, `/api/customer/status`, `/api/products`, `/api/orders`, `/api/orders/{id}`. 핸들러는 `FleetRepository`(스레드 안전)만 호출. TestClient + 실기동(바인드/응답/정지) 검증 완료.
- [x] DB 명령 엔드포인트 추가: `POST /api/orders`(주문 생성, 재고 검증/차감), `POST /api/orders/{id}/complete`, `POST /api/admin/products`, `PATCH /api/admin/products/{id}`, `PATCH /api/admin/products/{id}/stock`, `POST /api/admin/pickup-slots`, `POST /api/admin/exceptions/{id}/resolve`. `RepoError(status_code)`로 404/400 매핑. 요청 검증은 `fleet_api_schemas.py`. TestClient로 성공/검증실패/not-found 검증 완료.
- [x] ROS 핸드오프 명령(emergency-stop/resume): `POST /api/admin/emergency-stop`, `/api/admin/resume`. `FleetRepository.apply_emergency_stop/apply_resume`가 DB 전이(로봇 EMERGENCY_STOP, RUNNING→PAUSED / PAUSED→RUNNING + task_event)를 담당하고, 노드 `trigger_emergency_stop`가 DB 전이 + `RobotCommandGateway` 전파 + `task_manager.handle_*`를 묶는다. WS listener 경로(`_send_emergency_stop`)와 전파 로직(`_propagate_emergency`)을 공유. DB 전이는 실 DB 검증 완료(로봇 전파는 기존 WS 경로와 동일 코드).
- [ ] LLM 명령(`POST /api/admin/llm/messages`): 현재 `llm_client` stub 이 web 잔류. LLM 담당 연결과 함께 처리(보류).
- [x] 실시간 WebSocket push 추가: `WS /api/admin/ws/status`, `/api/customer/ws/status`. 연결 시 즉시 스냅샷 1회 + 이벤트 루프의 **주기 push 루프**(기본 1초, 연결 클라이언트 있을 때만, DB 조회는 `run_in_executor`로 루프 밖). 설계 선택: 상태 변경이 여러 스레드(API/TaskManager)에서 일어나 쓰기 지점마다 훅 거는 방식은 침습적/스레드 경계 문제가 커서, 출처 무관하게 항상 최신 반영하는 주기 push 채택. `api_push_interval_sec` 파라미터. 실기동+실 WebSocket 클라이언트로 on-connect/주기 push 검증 완료.
- [ ] 웹에서 DB 연결 코드 제거 (`database.py`, 모델 import, shim). 웹에 DB 코드 0줄.
- [ ] 웹의 쓰기/조회를 Fleet Manager API 경유로 교체.
- [ ] Emergency/Resume의 `/api/fleet/ws/events` 우회 경로 정리.

### Phase 5 — 정리

- [ ] 사용되지 않게 된 `/api/fleet/*` 엔드포인트와 `ControlServerClient` HTTP 코드 제거.
- [ ] 아키텍처 문서(`docs/3_System_Architecture.pdf`) ver_3.0 갱신: Control Server를 read-only 프런트로 표기, DB 화살표는 Fleet Manager에만 연결.
