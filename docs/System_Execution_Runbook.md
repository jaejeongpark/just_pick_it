...
```bash
cd ~/just_pick_it
DEMO_MOCK_PICKY=0 DEMO_MOCK_COBOT=1 bash scripts/demo/run_fake_robot_servers.sh
```
...
```bash
cd ~/just_pick_it
DEMO_MOCK_PICKY=1 DEMO_MOCK_COBOT=0 bash scripts/demo/run_fake_robot_servers.sh
```
...
```bash
cd ~/just_pick_it
USE_DDS=0 DEMO_MOCK_PICKY=1 DEMO_MOCK_COBOT=1 bash scripts/demo/run_fake_robot_servers.sh
```
...
- 실제 PICKY와 fake server를 같이 쓸 때는 `DEMO_MOCK_PICKY=0`을 붙인다.
- fake 없이 전체 실로봇 테스트를 할 때는 `scripts/demo/run_fake_robot_servers.sh`를 켜지 않는다.
- 실제 COBOT과 fake server를 같이 쓸 때는 `DEMO_MOCK_COBOT=0`을 붙인다.
