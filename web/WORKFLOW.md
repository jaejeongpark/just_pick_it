...
- 각 Robot Control Node는 자기 로봇의 task를 실행하지만, 최종 목적은 주문을 다음 상태로 보내는 것입니다.
...
Control Server / Web API
  ...
  - 각 Robot Control Node 또는 Control Bridge가 보낸 상태 업데이트를 DB에 반영
  - DB 변경 API commit 후 고객/관리자 UI에 WebSocket broadcast

Robot Control Node / ROS2
  - 각 담당자가 맡은 로봇 작업을 실행
  - 자기 로봇의 task queue를 조회하고 작업 상태를 판단
...
Robot Control Node, Control Bridge, Vision Server, LLM Server는 DB에 직접 접근하지 않습니다.
...
```text
Robot Control Node / Control Bridge
  - 자기 로봇의 task queue 조회
  - 작업 시작/완료/실패 판단
  - 주문/작업/로봇/픽업슬롯 상태 전이 보고
  - 결정 결과를 /api/fleet/* 로 보고

Vision Server
  - 인식/검수/감지 결과 생성
  - 단순 예외 알림은 /api/fleet/exceptions 로 보고 가능
  - 작업 성공/실패 판단이 필요한 결과는 해당 Robot Control Node가 받아서 task/order 상태로 보고

LLM Server
  - 자연어 명령을 구조화
  - Control Server가 LLM 응답을 받아 Admin UI에 표시하거나 로봇 담당 노드가 사용할 명령 형태로 전달

Control Server
  - 외부 시스템이 보낸 결과를 저장
  - DB 변경 후 WebSocket broadcast
  - 각 로봇 담당 노드의 실행 판단을 대신하지 않음
```
...
```text
COBOT1 : 상품 선별
AMR1   : 상차 대기 + 상품 운반 + 하차 대기
COBOT2 : 상품 검수 + 픽업 슬롯 하차
```
...
```text
주문 1건당 AMR 1대를 배정한다.
같은 주문의 STANDBY_LOAD / DELIVERY / STANDBY_UNLOAD task는 같은 AMR이 담당한다.
다른 주문은 담당 노드/Control Bridge 정책에 따라 다른 AMR을 배정할 수 있다.
```
...
이후 담당 Robot Control Node 또는 Control Bridge가 주문에 필요한 task를 생성/시작합니다.

호출 예:

```text
POST  /api/fleet/tasks                 STANDBY_LOAD task 생성
POST  /api/fleet/tasks                 SORTING task 생성
POST  /api/fleet/tasks                 DELIVERY task 생성
POST  /api/fleet/tasks                 STANDBY_UNLOAD task 생성
POST  /api/fleet/tasks                 INSPECTION task 생성
POST  /api/fleet/tasks                 UNLOAD task 생성
PATCH /api/fleet/tasks/{standby_load_id} {"status":"RUNNING","assigned_robot_id":"AMR1"}
POST  /api/fleet/tasks/{standby_load_id}/events
PATCH /api/fleet/robots/AMR1             {"status":"STANDBY","current_task_id":standby_load_id}
PATCH /api/fleet/orders/7                {"status":"ORDER_WAIT"}
```
...
```text
orders
  status: ORDER_RECEIVED -> ORDER_WAIT

order_item
  status: WAITING

task
  STANDBY_LOAD   : RUNNING, assigned_robot_id=AMR1
  SORTING        : QUEUED,  assigned_robot_id=COBOT1
  DELIVERY       : QUEUED,  assigned_robot_id=AMR1
  STANDBY_UNLOAD : QUEUED,  assigned_robot_id=AMR1
  INSPECTION     : QUEUED,  assigned_robot_id=COBOT2
  UNLOAD         : QUEUED,  assigned_robot_id=COBOT2

robot
  AMR1.status: IDLE -> STANDBY
  AMR1.current_task_id: STANDBY_LOAD task id
...
Admin UI
  Orders: ORD-0007 / 주문 대기
  Robots: AMR1 / 상차 대기 task 진행 중
```

### 2. AMR이 상차 대기존에 도착하고 선별 시작
...
PATCH /api/fleet/tasks/102      {"status":"RUNNING","assigned_robot_id":"COBOT1"}
PATCH /api/fleet/robots/COBOT1  {"status":"SORTING","current_task_id":102}
...
```text
orders
  status: ORDER_WAIT -> SORTING

order_item
  status: WAITING

task
  STANDBY_LOAD : RUNNING -> SUCCESS
  SORTING      : QUEUED  -> RUNNING

robot
  AMR1.status: STANDBY -> IDLE
...
Admin UI
  ...
  Robots: COBOT1 / 선별 task 진행 중
```

### 3. COBOT1 담당 노드가 선별 완료 후 운반 단계로 전환

호출:

```text
PATCH /api/fleet/tasks/102      {"status":"SUCCESS"}
PATCH /api/fleet/robots/COBOT1  {"status":"IDLE","current_task_id":null}
PATCH /api/fleet/tasks/103      {"status":"RUNNING","assigned_robot_id":"AMR1"}
PATCH /api/fleet/robots/AMR1    {"status":"DELIVERING","current_task_id":103}
...
PATCH /api/fleet/robots/AMR1         {"status":"STANDBY","current_task_id":104}
PATCH /api/fleet/orders/7            {"status":"DELIVERING"}
```
...
task
  ...
  STANDBY_UNLOAD : QUEUED  -> RUNNING

robot
  AMR1.status: DELIVERING -> STANDBY
  AMR1.current_task_id: STANDBY_UNLOAD task id
```
...
PATCH /api/fleet/tasks/105           {"status":"RUNNING","assigned_robot_id":"COBOT2"}
PATCH /api/fleet/robots/COBOT2       {"status":"INSPECTING","current_task_id":105}
...
orders
  ...
  pickup_slot_id: null -> 1

task
  STANDBY_UNLOAD : RUNNING -> SUCCESS
...
```text
PATCH /api/fleet/tasks/105           {"status":"SUCCESS"}
PATCH /api/fleet/robots/COBOT2       {"status":"IDLE","current_task_id":null}
PATCH /api/fleet/tasks/106           {"status":"RUNNING","assigned_robot_id":"COBOT2"}
PATCH /api/fleet/robots/COBOT2       {"status":"UNLOADING","current_task_id":106}
PATCH /api/fleet/orders/7            {"status":"DELIVERING"}
```
...
robot
  COBOT2.status: INSPECTING -> UNLOADING
```
...
```text
PATCH /api/fleet/tasks/106       {"status":"SUCCESS"}
PATCH /api/fleet/robots/COBOT2   {"status":"IDLE","current_task_id":null}
...
robot
  COBOT2.status: UNLOADING -> IDLE
  COBOT2.current_task_id: null
...
각 로봇 담당 노드는 자기 로봇의 상태를 판단합니다.
Control Server는 담당 노드가 보낸 상태를 검증한 뒤 DB에 반영하고 UI에 보여줍니다.
...
상태 판단과 다음 동작 결정은 각 Robot Control Node 또는 Control Bridge 쪽에서 하고, Control Server는 API로 받은 결과를 저장하는 역할로 둡니다.
...
이 구조에서는 다음 판단을 각 Robot Control Node 또는 Control Bridge가 담당합니다.
...
Admin 수동 수정
  ...
  - 실제 로봇 노드 없이도 화면 테스트 가능

Robot Runtime API
  - 실제 Robot Control Node/ROS2 또는 Control Bridge가 호출
  - 각 로봇 담당 노드가 판단한 상태를 Control Server가 DB에 반영
  - 실제 연동 기준
```
...
COBOT1 선별 중 gripper 실패
  -> COBOT1 담당 노드가 task 실패 상태와 예외를 보고
  -> task.status = FAILED
  -> orders.status = ERROR
  -> robot.status = ERROR
  -> exception_log 기록
...
1. RETURNING과 PARKING 구분
   ...
   - 둘 중 하나만 쓸지, 둘 다 쓸지 Robot Control Node 동작 기준으로 확정 필요
...
3. task_id와 queue 순서
   - task_id는 DB 식별자
   - 실제 우선순위/큐 순서는 담당 노드 또는 Control Bridge 정책으로 판단
```
...
STANDBY
  - 올바른 표기다.
  - robot.status에서는 AMR 상하차 대기 상태로 사용한다.
  - task_type에서는 STANDBY_LOAD / STANDBY_UNLOAD 형태로 사용한다.
